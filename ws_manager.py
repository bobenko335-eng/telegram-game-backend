...
class WSManager:
    ...
    # ---- global feed (виден всем игрокам) ----
    async def connect_feed(self, ws: WebSocket):
        await ws.accept()
        async with self._lock:
            self._feed_connections.add(ws)

    async def disconnect_feed(self, ws: WebSocket):
        async with self._lock:
            self._feed_connections.discard(ws)

    async def broadcast_feed(self, payload: dict):
        conns = list(self._feed_connections)
        dead = []
        for ws in conns:
            try:
                await ws.send_json(payload)
